...
3. You can execute the tests of the program via the command `pytest -W ignore`, but **ensure that you are using Python version 3.8.10** when you execute the test.
   - If you don't see the expected test output as described below, you may install [Docker](https://www.docker.com/) and run the tests in a Docker container. In this case, use the command `docker run -it --rm --name run-test -v "$PWD":/root/ -w /root/ python:3.8 pytest -W ignore`.
