...
1. Of course, [Git](https://git-scm.com/) must be installed.
   - Clone this repository. `git clone https://github.com/coinse/cs350-git-challenge.git`
...
2. Install required packages via the command `pip install -r requirements.txt`.
3. You can execute the tests of the program via the command `pytest -W ignore`, but **ensure that you are using Python version 3.8.10** when you execute the test.
   - If you don't see the expected test output as described below, you may install [Docker](https://www.docker.com/) and run the tests in a Docker container. In this case, use the command `docker run -it --rm --name run-test -v ./:/root/ -w /root/ python:3.8 bash -c "pip install -r requirements.txt && pytest -W ignore"`.
...
pasta/augment/import_utils_test.py ..................................    [ 14%]
pasta/augment/inline_test.py .......                                     [ 17%]
pasta/augment/rename_test.py ..........                                  [ 21%]
pasta/base/annotate_test.py .............s.ss..............s.......s.... [ 39%]
s.........s...ss.s.......s.....................s.......s....s.........s. [ 69%]
..ss.s.......s...........                                                [ 80%]
pasta/base/ast_utils_test.py ......                                      [ 82%]
pasta/base/codegen_test.py ..........sss....s.                           [ 90%]
pasta/base/scope_test.py .................                               [ 97%]
pasta/base/test_utils_test.py .....                                      [100%]

======================= 216 passed, 23 skipped in 0.21s ========================
```
...
```
docker run -it --rm --name run-test -v ./:/root/ -w /root/ python:3.8 bash -c "pip install -r requirements.txt && pytest -W ignore"
```
...
Looking through the commit history, the commit with the message "Add test case for BinOp" adds the failed test case, `test_auto_format_binop`. _Then, which past commit was the culprit that caused this test to fail?_ A naive way to find the bug-introducing commit is to manually check out each commit and run the test. However, this is quite tedious. Instead, we strongly recommend you to try using `git bisect` to reduce manual inspection.
...
- `git cherry-pick -n <commit_hash>` applies a change contained in the commit without _committing_ it.
